...
DEFAULT_CONFIG_PATH = ROOT / "example" / "hom_swapping_validation" / "configs" / "hom_three_overlap_batch_config.ini"
...
DEFAULT_SYNC_EXCLUDES = [
    ...
    ".mypy_cache",
    ".ipynb_checkpoints",
    "example/hom_swapping_validation/results",
]

from example.hom_swapping_validation.simulator import (  # noqa: E402
    CoincidenceConfig,
    SequenceHOMConfig,
...
def _run_task(task: Dict[str, Any]) -> Dict[str, Any]:
    ...
    bsm_cfg = CoincidenceConfig(**task["bsm_cfg"])
...
def _build_tasks(args: argparse.Namespace) -> List[Dict[str, Any]]:
    ...
    bsm_cfg = CoincidenceConfig(window_ps=int(args.bsm_window_ps), offset_ps=int(args.bsm_offset_ps)).__dict__.copy()
...
def _aggregate_runs(
    ...
    for keys, group in df.groupby(group_cols, sort=True):
        ...
        for count_col, rate_col in [
            ("bsm_twofold_count", "bsm_twofold_rate_per_pulse"),
