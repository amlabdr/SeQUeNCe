# BSM-Only AI Diagnosis Dataset

This package generates HOM/BSM monitoring datasets for fault diagnosis. The
model-eligible physical features are derived only from the two BSM detector
...
simulator uses `herald_mode="bsm_only"`, so idler detections, heralded
...
- `temperature_drift`: applies a signed temperature shift to one randomly
  selected signal fiber. No timing correction is supplied to the BSM.
...
- `source_clock_drift`: creates a signed relative source-timing walk-off before
  the BSM. The offset grows with the simulated time elapsed since onset.
...
- `bsm_observable_windows.csv`: AI input table, one row per episode window.
...
`write_raw_streams=false` is the normal mode. If enabled, raw BSM timestamps
...
`bsm_observable_windows.csv` contains:

- BSM1 and BSM2 singles counts and rates
- summed singles rate
- BSM twofold count and rate
...
- BSM1 and BSM2 interarrival mean, standard deviation, minimum, and maximum
...
```powershell
python example/hom_swapping_validation/ai_bsm_dataset/generate_bsm_dataset.py `
  --config example/hom_swapping_validation/ai_bsm_dataset/configs/bsm_ai_dataset_config.ini `
  --target local
```
...
python example/hom_swapping_validation/ai_bsm_dataset/generate_bsm_dataset.py `
  --config example/hom_swapping_validation/ai_bsm_dataset/configs/bsm_ai_dataset_config.ini `
  --target local `
  --output-dir example/hom_swapping_validation/ai_bsm_dataset/generated/smoke `
  --labels healthy temperature_drift `
  --dataset-size 2 `
...
```bash
/home/ana35/.venvs/sequence-bsm/bin/python \
  example/hom_swapping_validation/ai_bsm_dataset/generate_bsm_dataset.py \
  --config example/hom_swapping_validation/ai_bsm_dataset/configs/bsm_ai_dataset_config.ini \
  --target local \
