"""Generate BSM-only AI diagnosis datasets from HOM/swapping simulations."""
...
from example.hom_swapping_validation.ai_bsm_dataset.config import (  # noqa: E402
    FAULT_LABELS,
    DatasetConfig,
    default_output_root,
    load_dataset_config,
)
from example.hom_swapping_validation.ai_bsm_dataset.scenarios import build_scenario  # noqa: E402
from example.hom_swapping_validation.ai_bsm_dataset.simulator import simulate_episode  # noqa: E402


DEFAULT_CONFIG = ROOT / "example" / "hom_swapping_validation" / "ai_bsm_dataset" / "configs" / "bsm_ai_dataset_config.ini"
DEFAULT_SYNC_EXCLUDES = [
    ...
    ".mypy_cache",
    ".ipynb_checkpoints",
    "example/hom_swapping_validation/ai_bsm_dataset/generated",
]
...
def _validation_summary(observable_rows: list[dict], label_rows: list[dict]) -> list[dict]:
    ...
    for label_row in label_rows:
        ...
        row = {
            ...
            "pre_bsm_twofold_rate_hz": avg(pre, "bsm_twofold_rate_hz"),
            "post_bsm_twofold_rate_hz": avg(post, "bsm_twofold_rate_hz"),
            "pre_bsm1_rate_hz": avg(pre, "bsm1_rate_hz"),
            "post_bsm1_rate_hz": avg(post, "bsm1_rate_hz"),
            "pre_bsm2_rate_hz": avg(pre, "bsm2_rate_hz"),
            "post_bsm2_rate_hz": avg(post, "bsm2_rate_hz"),
...
def _write_final_outputs(
    ...
    observable_df.to_csv(output_dir / "bsm_observable_windows.csv", index=False)
    ...
    metadata = {
        ...
        "note": "AI inputs are BSM-only. Hidden truth contains simulator-only labels and must not be used as operational input.",
    }
...
def _make_repo_archive() -> Path:
    tmp = tempfile.NamedTemporaryFile(prefix="bsm_ai_repo_", suffix=".tar.gz", delete=False)
...
def _sync_project_to_cluster(cfg: DatasetConfig, remote_repo: str) -> None:
    ...
    remote_tmp = f"/tmp/bsm_ai_repo_{os.getpid()}.tar.gz"
    try:
...
def _write_slurm_scripts(cfg: DatasetConfig, args: argparse.Namespace, remote_repo: str) -> tuple[Path, Path, Path | None]:
    ...
    array_script = output_dir / "bsm_ai_dataset_array.slurm"
    combine_script = output_dir / "bsm_ai_dataset_combine.slurm"
    benchmark_script = output_dir / "bsm_ai_dataset_benchmark.slurm" if args.benchmark_one else None
...
#SBATCH --output={out_rel}/bsm_ai_%A_%a.out
#SBATCH --error={out_rel}/bsm_ai_%A_%a.err

set -euo pipefail
cd {remote_repo}
{module_line}
mkdir -p {out_rel}
echo "Running BSM-AI shard $SLURM_ARRAY_TASK_ID/{array_tasks} cpus=$SLURM_CPUS_PER_TASK"
...
""",
    ...
    combine_script.write_text(
        f"""#!/bin/bash
#SBATCH --job-name=bsm_ai_combine
{partition_line}
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=1
{time_line}
#SBATCH --output={out_rel}/bsm_ai_combine_%j.out
#SBATCH --error={out_rel}/bsm_ai_combine_%j.err
...
""",
    ...
    if benchmark_script is not None:
        ...
        benchmark_script.write_text(
            f"""#!/bin/bash
#SBATCH --job-name=bsm_ai_bench
{partition_line}
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=1
{time_line}
#SBATCH --output={out_rel}/bsm_ai_benchmark_%j.out
#SBATCH --error={out_rel}/bsm_ai_benchmark_%j.err
...
def _clean_old_logs(cfg: DatasetConfig, output_dir: Path, remote_repo: str | None = None) -> None:
    for pattern in ("bsm_ai_*.out", "bsm_ai_*.err"):
        for path in output_dir.glob(pattern):
            path.unlink(missing_ok=True)
    if remote_repo:
        remote_output = f"{remote_repo.rstrip('/')}/{_repo_relative_posix(output_dir)}"
        cmd = f"mkdir -p {_shell_quote(remote_output)} && rm -f {_shell_quote(remote_output)}/bsm_ai_*.out {_shell_quote(remote_output)}/bsm_ai_*.err"
...
def _collect_outputs(cfg: DatasetConfig, output_dir: Path, remote_repo: str, tag: str = "latest") -> None:
    remote_output = f"{remote_repo.rstrip('/')}/{_repo_relative_posix(output_dir)}"
    remote_tmp = f"/tmp/bsm_ai_outputs_{tag}.tar.gz"
    local_tmp = output_dir / f"bsm_ai_outputs_{tag}.tar.gz"
...
def main() -> None:
    ...
    print(
        f"Generating BSM dataset episodes={len(jobs)} shard_episodes={len(shard_jobs)} "
