"""Fault scenario generation for HOM detector diagnostics."""
...
class WindowState:
    ...
    faulty_source: str = ""
    faulty_arm: str = ""
    faulty_hom_detector: str = ""


@dataclass(frozen=True)
class HOMDiagnosisScenario:
    episode_id: str
    setup_id: str
    fault_class: str
    seed: int
...
def build_scenario(label: str, episode_index: int, cfg: DatasetConfig, seed: int) -> HOMDiagnosisScenario:
    ...
    arm_faults = {"attenuation_loss", "polarization_drift", "raman_noise", "temperature_change"}
    ...
    faulty_hom_detector = str(rng.choice(["HOM1", "HOM2"])) if label in detector_faults else ""
    target = 0.0
    states: list[WindowState] = []

    if label == "temperature_change":
    ...
    for window_index in range(windows):
        ...
        kwargs = {
            ...
            "faulty_hom_detector": faulty_hom_detector if label == "sync_issue" else "",
        }
    ...
    return HOMDiagnosisScenario(
        ...
        faulty_source=faulty_source,
        faulty_arm=faulty_arm,
        faulty_hom_detector=faulty_hom_detector,
