"""HOM-only episode simulation for AI diagnosis datasets."""
...
from example.hom_validation.simulator import (
    CoincidenceConfig,
    SequenceHOMConfig,
...
from .features import add_temporal_derived_features, hom_observable_features
from .scenarios import HOMDiagnosisScenario, WindowState
...
def _apply_clock_desynchronization(
    ...
    *,
    ...
    """Apply detector-clock jitter to one HOM timestamp stream.

    This intentionally changes the observable HOM timestamps. It is not a
...
def _window_sequence_config(window: WindowState, cfg: DatasetConfig, seed: int) -> SequenceHOMConfig:
    ...
    return SequenceHOMConfig(
        ...
        detector_eff_hom=float(setup.detector_eff_hom),
        detector_eff_herald=1.0,
        detector_jitter_ps=float(setup.detector_jitter_ps),
        detector_dark_hz_hom=float(setup.detector_dark_hz_hom),
...
def simulate_window(
    ...
    # drift is a real relative source-timing walk-off before the HOM, not a
    ...
    df, streams = run_hom_sequence_delay_scan(
        delays_ps=[delay_ps],
        cfg=sim_cfg,
        hom_cfg=CoincidenceConfig(window_ps=int(cfg.histogram.coincidence_window_ps), offset_ps=0),
        store_streams=True,
    )
    # Stream keys are part of the shared simulator's legacy API.
    hom1 = streams[delay_ps]["HOM1_ps"]
    hom2 = streams[delay_ps]["HOM2_ps"]
    sync_rng = np.random.default_rng(int(seed) + 7919)
    if window.faulty_hom_detector == "HOM1":
        hom1_observed = _apply_clock_desynchronization(
            hom1,
        ...
        hom1_observed = hom1
        hom2_observed = _apply_clock_desynchronization(
            hom2,
            jitter_ps=float(window.true_sync_jitter_ps),
            rng=sync_rng,
        )
    features, raw_hist = hom_observable_features(
        hom1_ps=hom1_observed,
        hom2_ps=hom2_observed,
    ...
    truth = {
        ...
        **{
            ...
            if key not in {"faulty_source", "faulty_arm", "faulty_hom_detector"}
        },
    ...
    if include_raw:
        raw = {
            ...
            "window_index": int(window.window_index),
            "histogram": raw_hist,
            "hom1_ps": [int(v) for v in hom1_observed],
            "hom2_ps": [int(v) for v in hom2_observed],
        }
