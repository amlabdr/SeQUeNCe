...
def hom_twofold_events(
    hom1_ps: Sequence[int],
    hom2_ps: Sequence[int],
    cfg: CoincidenceConfig,
    assume_sorted: bool = True,
) -> List[Tuple[int, int, int]]:
    """Return matched HOM twofold events as (t1, t2, t_ref_midpoint)."""
...
def fourfold_count_from_hom(
    hom_events: Sequence[Tuple[int, int, int]],
    ...
    """Count fourfold events by validating extra detector hits around HOM events."""
    total = 0
    for _, _, tref in hom_events:
        has_a = event_has_hit(
...
class _PhotonSinkNode(Node):
    """Passive idler sink used when the run is HOM-only.
...
def run_hom_sequence_delay_scan(
    ...
    hom_cfg: CoincidenceConfig = CoincidenceConfig(window_ps=120, offset_ps=0),
    ...
    - HOMInterferenceNode at middle HOM
    - PolarizationAnalyzerNode on local idler arms for state-projecting heralds,
      or passive idler sinks when cfg.herald_mode == "hom_only"
    """
    rows: List[Dict[str, float]] = []
    streams: Dict[int, Dict[str, np.ndarray]] = {}
    herald_mode = str(cfg.herald_mode).lower()
    if herald_mode not in {"projected", "hom_only"}:
        raise ValueError("cfg.herald_mode must be 'projected' or 'hom_only'")
    ...
    for idx, delay_ps in enumerate(delays_ps):
        ...
        hom = HOMInterferenceNode(
            ...
            timeline,
            {
                ...
                "detector_efficiency": float(cfg.detector_eff_hom),
                "detector_jitter_ps": float(cfg.detector_jitter_ps),
                "dark_count_rate_hz": float(cfg.detector_dark_hz_hom),
        ...
        # signal = port0 -> one fiber -> optional waveplate relay -> HOM.
        ...
        hom_ref_delay_ps = max(signal_a_ref_delay_ps, signal_b_ref_delay_ps)
        rel_a = CoincidenceConfig(
            window_ps=int(herald_a_rel.window_ps),
            offset_ps=(
                herald_a_delay_ps - hom_ref_delay_ps
        ...
        rel_b = CoincidenceConfig(
            ...
            offset_ps=(
                herald_b_delay_ps - hom_ref_delay_ps
        ...
        hom0, hom1 = hom.get_click_times(flush=True, include_dark_counts=True, reset=False)
        if herald_mode == "projected":
        ...
        hom_events = hom_twofold_events(hom0, hom1, hom_cfg, assume_sorted=True)
        if herald_mode == "hom_only":
            rows.append(
                {
                    "delay_ps": int(delay_ps),
                    "hom1_count": int(len(hom0)),
                    "hom2_count": int(len(hom1)),
                    "hom_twofold_count": int(len(hom_events)),
                    "hom_twofold_rate_per_pulse": float(len(hom_events) / max(1, cfg.pulses_per_delay)),
            ...
            if store_streams:
                streams[int(delay_ps)] = {
                    "HOM1_ps": _sorted_array(hom0, assume_sorted=True),
                    "HOM2_ps": _sorted_array(hom1, assume_sorted=True),
                }
            continue

        if herald_mode == "projected":
            hom_fourfold = fourfold_count_from_hom(
                hom_events,
            ...
            selected_fourfold = fourfold_count_from_hom(
                hom_events,
            ...
            hh_fourfold = fourfold_count_from_hom(
                hom_events, herald_a_readout.h_times, herald_b_readout.h_times, rel_a, rel_b, assume_sorted=True
            )
            hv_fourfold = fourfold_count_from_hom(
                hom_events, herald_a_readout.h_times, herald_b_readout.v_times, rel_a, rel_b, assume_sorted=True
            )
            vh_fourfold = fourfold_count_from_hom(
                hom_events, herald_a_readout.v_times, herald_b_readout.h_times, rel_a, rel_b, assume_sorted=True
            )
            vv_fourfold = fourfold_count_from_hom(
                hom_events, herald_a_readout.v_times, herald_b_readout.v_times, rel_a, rel_b, assume_sorted=True
            )
        else:
            hom_fourfold = 0
            selected_fourfold = 0
